<template>
  <div class="home">
    <h1>Search and rate TV Shows</h1>
    <p class="lead">You can search, rate or add a show to your favorites.</p>
    <div class="lead m-auto mb-5" style="max-width: 500px">
      <div class="mb-3">
        <span class="p-input-icon-left p-input-icon-right w-100">
            ...
            <InputText type="text" class="p-inputtext-lg w-100"  placeholder="Search for a show.." v-model="query"/>
        </span>
    ...
    <div class="row mb-3">
      <div class="col-12 text-start">
        <ul class="nav mfilters">
          <li class="nav-item">
            <a class="nav-link fw-bold text-muted" :class="{ 'active': mfilter === 'top_rated'}" href="#" @click.prevent="$store.dispatch('setShowFilter', 'top_rated')">
              Top rated shows
              <i class="fa-solid fa-star text-warning  ms-1"></i>
              </a>
          </li>
          <li class="nav-item">
            <a class="nav-link fw-bold text-muted" :class="{ 'active': mfilter === 'popular'}" href="#" @click.prevent="$store.dispatch('setShowFilter', 'popular')">
              Popular shows
              <i class="fa-solid fa-star text-warning  ms-1"></i>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link fw-bold text-muted" :class="{ 'active': mfilter === 'airing_today'}" href="#" @click.prevent="$store.dispatch('setShowFilter', 'airing_today')">
              Airing today
              <i class="fa-solid fa-star text-warning  ms-1"></i>
            </a>
          </li>
          <li class="nav-item">
            <a class="nav-link fw-bold text-muted" :class="{ 'active': mfilter === 'on_the_air'}" href="#" @click.prevent="$store.dispatch('setShowFilter', 'on_the_air')">
              On TV now
              <i class="fa-solid fa-star text-warning  ms-1"></i>
            </a>
          </li>
      ...
      <div class="col col-sm-12 col-md-4 col-lg-3" v-for="(movie, key) in movies" :key="key">
        ...
        >
          <div class="card bg-dark text-white mb-1">
            <img :id="movie.id" :src="`https://www.themoviedb.org/t/p/w220_and_h330_face${movie.poster_path}`" @error="imgerror" class="card-img poster" :alt="movie.name">
          </div>
          <div class="card bg-secondary mb-5 subcard">
            <div class="card-body">
              <div class="text-truncate"><strong>{{ movie.name }}</strong></div>
              <div class="row">
                <div class="col col-4 small text-light opacity-50">
                  <strong class="me-3">{{ new Date(movie.first_air_date).getFullYear() }}</strong>
                </div>
  ...
  <Dialog @show="toggleBody('hidden')" @hide="toggleBody('auto')" v-model:visible="dialog" :breakpoints="{'960px': '75vw', '640px': '100vw'}" :style="{width: '60vw'}" :modal="true" :draggable="false">
    <template #header v-if="dialogContent !== null">
      <h4>{{ dialogContent.name }} ({{ new Date(dialogContent.first_air_date).getFullYear() }})</h4>
    </template>
    <div class="card mb-3 shadow-none bg-dark" style="width: 100%; border: none;">
      <div class="row g-0">
        <div class="col-md-4 p-0 m-0">
          <img :src="`https://www.themoviedb.org/t/p/w220_and_h330_face${dialogContent.poster_path}`" @error="imgerror" class="img-fluid rounded-start w-100" :alt="dialogContent.name">
        </div>
        <div class="col-md-8 text-start">
          <div class="card-body">
            <h5 class="card-title">{{ dialogContent.name }} ({{ new Date(dialogContent.first_air_date).getFullYear() }})</h5>
            <p class="card-text">
...
export default {
  ...
  setup() {
    ...
    const topMovies = computed(() => store.state.shows);
    const mfilter = computed(() => store.state.showFilters);
    ...
    const searchMovie = async (event) => {
      ...
      const search = await Movie.queryShow(query.value);
        setTimeout(() => {
    ...
    const nextPageQuery = async () => {
      ...
      if(customFilter !== null) {
          movieresults = await Movie.discoverShow(customFilter, queryPageNo.value);
        } else {
          movieresults = await Movie.queryShow(query.value, queryPageNo.value);
        }
    ...
    const search = (query) => {
      ...
      delay = setTimeout(async () => {
        ...
        if(customFilter !== null) {
          ...
          movieresults = await Movie.queryShow(query, queryPageNo.value);
        }
